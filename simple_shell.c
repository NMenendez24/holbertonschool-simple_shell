...
{	char *buff = NULL, *buffdup = NULL, *token = NULL,
	...
	while (1)
	{
		...
		for (ac = 1; ac <= argcount; ac++)
			aux = strtok(NULL, " \\\t\n"), av[ac] = aux;
		pid_check = fork();
		(pid_check == 0) ? execve(av[0], av, environ) : 0;
		wait(&status);
		free(buffdup), free(av), (com_path) ? free(com_path) : (void) 0; }}
